...
export function groupEvents(dayEvents: IEvent[]) {
  ...
  for (const event of sortedEvents) {
    ...
    for (const group of groups) {
      const lastEventEnd = parseISO(group[group.length - 1].endDate);
...
export function isWorkingHour(
  ...
  const dayHours = workingHours[day.getDay()];
...
export function getVisibleHours(
  ...
  const hours = Array.from(
    ...
    (_, index) => index + earliestEventHour,
  );
...
export function getCalendarCells(selectedDate: Date): ICalendarCell[] {
  ...
  const prevMonthCells = Array.from({ length: firstDayOfMonth }, (_, index) => ({
    day: daysInPrevMonth - firstDayOfMonth + index + 1,
    currentMonth: false,
    date: new Date(
      currentYear,
      currentMonth - 1,
    ...
    for (let index = 0; index < 3; index++) {
      if (
        eventDays.every((day) => {
          ...
          return dayPositions && !dayPositions[index];
        })
      ) {
        position = index;
        break;
...
export function filterCalendarEvents({
  ...
  return events.filter((event) => {
    ...
    if (view === "month" || view === "agenda") {
      ...
      return (
        eventStartDate <= monthEnd && eventEndDate >= monthStart && isUserMatch
      );
    }
...
export function splitSingleAndMultiDayEvents(events: IEvent[]) {
